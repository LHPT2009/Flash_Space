...
export default function TakephotoScreenRoom3({ navigation }) {
  ...
  const savePicture = async () => {
    if (image3.uri) {
      try {
        const asset = await MediaLibrary.createAssetAsync(image3.uri);
        showMessage({
          message: "Đã lưu hình  ✔",
          type: "success",
        });
    ...
    <View style={styles.container}>
      {!image3 ? (
        ...
        <View style={{ width: "100%", height: "90%" }}>
          ...
          ></View>
          <Image source={{ uri: image3.uri }} style={styles.camera} />
          <View
            style={{
              height: "20%",
              flexDirection: "row",
